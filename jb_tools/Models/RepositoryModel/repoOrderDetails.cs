...
public class z_repoOrderDetails : BaseClass
{
    ...
    /// <summary>
    /// 以 Dapper 來讀取單一訂單資料集合
    /// <summary>
    /// <param name="parentNo">訂單編號</param>
    /// <returns></returns>
    public List<OrderDetails> GetDapperDataOrderList(string parentNo)
    {
        using (DapperRepository dp = new DapperRepository())
        {
            string str_query = GetSQLSelect();
            str_query += " WHERE OrderDetails.ParentNo = @ParentNo ";
            str_query += GetSQLOrderBy();
            DynamicParameters parm = new DynamicParameters();
            parm.Add("ParentNo", parentNo);
            var model = dp.ReadAll<OrderDetails>(str_query , parm);
            return model;
        }
    }
